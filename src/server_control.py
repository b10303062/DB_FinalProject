...
def _user_join_room(pg_conn: psycopg.Connection, request: dict, cursor: Cursor, client_sock: socket.socket):
    ...
    if not join_room_lock.acquire(blocking=False):
        response = {
            "status": "FAIL",
            "errorMessage": "Another user is listing rooms. Please try again later."
        }
        sendall(client_sock, response)
        return
...
def _user_list_rooms(pg_conn: psycopg.Connection, request: dict, cursor: Cursor, client_sock: socket.socket):
    try:
        ...
        query = """
                SELECT "r"."room_id", "r"."room_name", "g"."game_name", "r"."creator_id", "u"."user_name", COUNT(uir.user_id) AS num_players, r.max_players
                FROM room r
                JOIN "user" u ON r.creator_id = u.user_id
                JOIN "game" AS "g" ON "r"."game_id" = "g"."game_id"
                LEFT JOIN user_in_room uir ON r.room_id = uir.room_id
                WHERE "r"."status" = 'Active'
                GROUP BY r.room_id, r.room_name, g.game_name, r.creator_id, u.user_name; """
        if game_id:
        ...
        for row in rows:
            response["rooms"].append({
                ...
                "numMembers": row["num_players"],
                "numMembersLimit": row["max_players"]
            })
...
def handle_request(pg_conn: psycopg.Connection, request: dict, cursor: Cursor, client_sock: socket.socket) -> int:
    ...
    match request_id:
        ...
        case 19:
            _user_leave_list_rooms(request, cursor, client_sock)
        case _:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pg_password", type = str, help = "Password for login the PostgreSQL server. (default = \"postgres\")", default = 'Ss0970375822')
    parser.add_argument("--pg_dbname", type = str, help = "Database to connect. (default = \"Steam-Together-fortest\")", default = "FinalProject")
